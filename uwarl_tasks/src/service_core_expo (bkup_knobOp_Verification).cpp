...
//                ROS_INFO("knob position:%.4f,%.4f,%.4f,   %.4f,%.4f,%.4f,%.4f",poseKnobFromWAM.position.x,poseKnobFromWAM.position.y,poseKnobFromWAM.position.z
...
//                desPoseHand.orientation = quatBHandGraspObjOrtn;
                fnCompGraspPoseKnob();
                ROS_INFO("Grasp Pose Computed!!!");
                desPoseHand = poseForGraspKnob;
...
//                srvKnobOperationWam.request.poseKnobWrtWAMbyCAM = poseKnobFromWAM;
//                for (int numj=0;numj<7;numj++){
//                    srvKnobOperationWam.request.jointsCur[numj] = jntact[numj];
//                }
                                tf2::toMsg(tfWamToKnob,poseKnobFromWAM);
                                srvKnobOperationWam.request.poseKnobWrtWAMbyCAM = poseKnobFromWAM;
                                srvKnobOperationWam.request.jointsCur[0] = 0;
                                srvKnobOperationWam.request.jointsCur[1] = -1;
                                srvKnobOperationWam.request.jointsCur[2] = 1;
                                srvKnobOperationWam.request.jointsCur[3] = 2.5;
                                srvKnobOperationWam.request.jointsCur[4] = -1;
                                srvKnobOperationWam.request.jointsCur[5] = 0.07;
                                srvKnobOperationWam.request.jointsCur[6] = 1;

                srvKnobOperationWam.request.cycleKnob = 1;
                srvKnobOperationWam.request.isRevDir = false;
                srv_knob_operation_wam.call(srvKnobOperationWam);
// 7. grasp open
                srv_hand_open_grsp.call(srvCmdEmpty);
                ros::Duration(2.0).sleep();
// 8. back to safe pose
...
//                srvKnobOperationWam.request.poseKnobWrtWAMbyCAM = poseKnobFromWAM;
                ...
                //                }
                                ...
                                srvKnobOperationWam.request.jointsCur[6] = 1;
                srvKnobOperationWam.request.cycleKnob = 1;
                srvKnobOperationWam.request.isRevDir = true;
                srv_knob_operation_wam.call(srvKnobOperationWam);
